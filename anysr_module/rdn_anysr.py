...
# from .arch_util import USConv2d
...
class USConv2d_fc2(nn.Conv2d):
    def __init__(
        ...
        super(USConv2d_fc2, self).__init__(
            in_channels,
            out_channels,
            kernel_size,
            stride=stride,
...
class RDB_any(nn.Module):
    def __init__(self, growRate0, growRate, nConvLayers, kSize=3):
        ...
        self.LFF = USConv2d(
            G0 + C * G, G0, 1, padding=0, stride=1, bias=True, us=[True, False]
        )
...
class RDN_anysr(nn.Module):
    def __init__(self, args):
        ...
        for i in range(self.D):
            self.RDBs.append(RDB_any(growRate0=G0, growRate=G, nConvLayers=C))
...
# @register("rdn-anysr")
def make_rdn(
    G0=64, RDNkSize=3, RDNconfig="B", scale=2, no_upsampling=False, rgb_range=1
):
    args = Namespace()
    args.G0 = G0
