...
def adain_fg(comp_feat, style_feat, mask):
    ...
    style_mean, style_std = get_foreground_mean_std(
        style_feat, downsample_mask_style
    )  # the style features
    fore_mean, fore_std = get_foreground_mean_std(
        comp_feat, mask
...
class VGG19HRNet(nn.Module):
    ...
    def downsample(self, image_tensor, width, height):
        image_upsample_tensor = torch.nn.functional.interpolate(
            image_tensor, size=[width, height]
        )
    ...
    def calc_style_loss_mulitple_fg_layer(self, combs, styles, mask, layer):
        ...
        for i in range(0, layer):
            ...
            downsample_mask_style = torch.ones(downsample_mask.size()).to(
                mask.device
            )  # 1-downsample_mask

            mu_cs, sigma_cs = get_foreground_mean_std(combs[i], downsample_mask)
            mu_target, sigma_target = get_foreground_mean_std(
                styles[i], downsample_mask_style
            )
            loss_i = self.mse_loss(mu_cs, mu_target) + self.mse_loss(
                sigma_cs, sigma_target
            )
            loss += loss_i
        return loss
    ...
    def cal_loss(self, final_output, comb_feats, style_feats, mask, layer):
        ...
        loss_s = self.calc_style_loss_mulitple_fg_layer(
            fine_feats, style_feats, mask, layer
        )
    ...
    def forward(self, comp, style, mask):
        ...
        loss_c_1, loss_s_1 = self.cal_loss(
            coarse_output1, comb_feats, style_feats, mask, 1
        )
        loss_c_2, loss_s_2 = self.cal_loss(
            coarse_output2, comb_feats, style_feats, mask, 2
        )
        loss_c_3, loss_s_3 = self.cal_loss(
            coarse_output3, comb_feats, style_feats, mask, 3
        )
        loss_c_4, loss_s_4 = self.cal_loss(
            coarse_output4, comb_feats, style_feats, mask, 4
        )
        ...
        loss_c_1, loss_s_1 = self.cal_loss(
            final_output1, comb_feats, style_feats, mask, 1
        )
        loss_c_2, loss_s_2 = self.cal_loss(
            final_output2, comb_feats, style_feats, mask, 2
        )
        loss_c_3, loss_s_3 = self.cal_loss(
            final_output3, comb_feats, style_feats, mask, 3
        )
        loss_c_4, loss_s_4 = self.cal_loss(
            final_output4, comb_feats, style_feats, mask, 4
        )
...
class ConvEncoder_D(nn.Module):
    def __init__(
        self, depth, ch, patch_number, norm_layer, batchnorm_from, max_channels
    ):
        ...
        for block_i in range(2, depth - 2):
            if block_i % 2:
                ...
                in_channels, out_channels = (
                    out_channels,
                    min(2 * out_channels, max_channels),
                )
...
class ConvDiscriminator(nn.Module):
    def __init__(
        ...
        self.depth = depth
        self.patch_number = patch_number
        self.encoder = ConvEncoder_D(
            depth, ch, patch_number, norm_layer, batchnorm_from, max_channels
        )
