...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        y = (
            ...
            .contiguous()
            .view(B // 2, H * W, 2, int(self.expand * C))
        )
        y = torch.cat([y[:, :, 0], y[:, :, 1]], 0).view(
            B, H, W, int(self.expand * C)
        )  # .view(B//2, 2*H, 2*W, -1)
...
class BiMambaBlock(nn.Module):
    def __init__(
        self, dim, depth, norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False
    ):
        super().__init__()
        self.dim = dim
        self.depth = depth
