...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.withmask:
            ...
            dsc = torch.cat(
                [
                    self.max_dsc(mask_in.reshape(b_, h_ * c_, c_)),
                    self.avg_dsc(mask_in.reshape((b_, h_ * c_, c_))),
                ],
                dim=-1,
            ).reshape(b_, h_, c_, 2)  # b,h,c,2
            mask = self.linear1(dsc)  # b,h,c,1
            mask = self.linear2(F.gelu(mask).transpose(-2, -1)).transpose(
                -2, -1
            )  # b, h, c, 1
            mask = F.sigmoid(mask)
...
class RealViformer(nn.Module):
    ...
    def __init__(
        ...
        masked=False,
        merge_head=1,
        merge_compress=False,
        merge_compress_factor=1.0,
        bias=False,
        LayerNorm_type="BiasFree",
        ch_compress=False,
        squeeze_factor=[1, 1, 1],
    ):
        super().__init__()
        # embedding
        ...
        self.attn_merge = ChannelAttentionBlock(
            dim_q=num_feat,
            dim_kv=num_feat,
            dim_out=num_feat,
            num_heads=merge_head,
            bias=False,
            LayerNorm_type="BiasFree",
            reduction=True,
            ch_compress=merge_compress,
            squeeze_factor=merge_compress_factor,
        )
        ...
        self.reduce_chan_level2 = nn.Conv2d(
            int(num_feat * 2**2), int(num_feat * 2**1), kernel_size=1, bias=bias
        )
