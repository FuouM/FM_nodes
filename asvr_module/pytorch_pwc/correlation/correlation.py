...
def get_or_compile_module(device_id, strKernel):
    if device_id not in compiled_modules:
        compiled_modules[device_id] = {}

    if strKernel not in compiled_modules[device_id]:
        module = cupy.RawModule(code=strKernel)
        compiled_modules[device_id][strKernel] = module

    return compiled_modules[device_id][strKernel]

def cupy_launch(strFunction, strKernel):
    device_id = get_device_id()
    module = get_or_compile_module(device_id, strKernel)
    function = module.get_function(strFunction)
    return function
