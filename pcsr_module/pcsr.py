...
from .fsrcnn import FSRCNN_net
from .other_utils import make_coord
from .sampler import LIIF_Sampler
...
class PCSR(nn.Module):
    ...
    def __init__(self, encoder_spec, heavy_sampler_spec, light_sampler_spec, classifier_spec):
        super().__init__()
        self.encoder = encoder_dict[encoder_spec["name"]](**encoder_spec["args"])
        in_dim = self.encoder.out_dim
        self.heavy_sampler = LIIF_Sampler(heavy_sampler_spec['args'], in_dim=in_dim, out_dim=3)
        self.light_sampler = LIIF_Sampler(light_sampler_spec['args'], in_dim=in_dim, out_dim=3)
        self.classifier = LIIF_Sampler(classifier_spec['args'], in_dim=in_dim, out_dim=2)
