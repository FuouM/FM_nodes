...
class FSRCNN_net(nn.Module):
    def __init__(
        self, input_channels=3, upscale=4, d=56, s=12, m=4, no_upsampling=False
    ):
        ...
        self.layers.append(
            nn.Sequential(
                nn.Conv2d(
                    in_channels=d, out_channels=s, kernel_size=1, stride=1, padding=0
                ),
                nn.PReLU(),
            )
        )
        ...
        self.layers.append(
            nn.Sequential(
                nn.Conv2d(
                    in_channels=s, out_channels=d, kernel_size=1, stride=1, padding=0
                ),
                nn.PReLU(),
            )
        )
