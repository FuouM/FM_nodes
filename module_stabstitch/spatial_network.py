...
def build_SpatialNet(net, input1_tensor, input2_tensor):
    ...
    overlap_w = torch.where(
        (mesh[..., 0] < img_w) & (mesh[..., 0] >= 0), overlap_one, overlap_zero
    )
    overlap_h = torch.where(
        (mesh[..., 1] < img_h) & (mesh[..., 1] >= 0), overlap_one, overlap_zero
    )
...
class SpatialNet(nn.Module):
    ...
    def forward(self, input1_tesnor, input2_tesnor):
        ...
        correlation_64 = self.cost_volume(
            feature_1_64, warp_feature_2_64, search_range=5, norm=False
        )  # bs, 9^2, 45, 80
